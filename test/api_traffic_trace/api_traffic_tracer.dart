...
class ScreenTraceReport {
  ...
  int get totalLatencyMs => apiCalls.fold(0, (sum, call) => sum + call.latencyMs);
  ...
  String toReport() {
    ...
    buffer.writeln('   - Duration: ${endTime.difference(startTime).inMilliseconds}ms');
    buffer.writeln();
    buffer.writeln('📡 ENDPOINTS CALLED:');
    for (final call in apiCalls) {
      final icon = call.statusCode >= 400 ? '❌' : (call.cacheStatus == 'HIT' ? '✅' : '🔄');
      buffer.writeln('   $icon ${call.method} ${call.endpoint}');
      buffer.writeln('      └─ Status: ${call.statusCode} | Latency: ${call.latencyMs}ms | Cache: ${call.cacheStatus}');
    }
    buffer.writeln();
    buffer.writeln('🔍 LOGIC CHECKS:');
    logicChecks.forEach((key, value) {
      final icon = value == true || value == 'PASS' ? '✅' : (value == false || value == 'FAIL' ? '❌' : '⚠️');
...
class ApiTrafficTracer {
  ...
  String generateFullReport(List<ScreenTraceReport> screens) {
    ...
    buffer.writeln();
    buffer.writeln('╔═══════════════════════════════════════════════════════════════════╗');
    buffer.writeln('║          API TRAFFIC TRACE - FULL SESSION REPORT                  ║');
