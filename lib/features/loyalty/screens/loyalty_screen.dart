...
class _LoyaltyScreenState extends State<LoyaltyScreen> {
  ...
  void initCall() {
    ...
    if (AuthHelper.isLoggedIn()) {
      ...
      if (kDebugMode) {
        debugPrint('🎁 [LoyaltyScreen] Loading loyalty transactions (offset=1)...');
      }
      ...
      scrollController.addListener(() {
        ...
        if (scrollController.position.pixels == scrollController.position.maxScrollExtent &&
          ...
          if (loyaltyController.offset < pageSize) {
            ...
            if (kDebugMode) {
              debugPrint('🎁 [LoyaltyScreen] Scrolled to bottom - loading page $newOffset');
              debugPrint('   📊 Current transactions: ${loyaltyController.transactionList?.length ?? 0}');
              debugPrint('   📄 Total pages: $pageSize');
            }
            loyaltyController.setOffset(newOffset);
            loyaltyController.showBottomLoader();
            loyaltyController.getLoyaltyTransactionList(newOffset.toString(), false);
          } else {
            if (kDebugMode) {
              debugPrint('🎁 [LoyaltyScreen] Reached last page - no more transactions to load');
            }
          }
        }
      });
      ...
      if (kDebugMode) {
        debugPrint('🎁 [LoyaltyScreen] User not logged in - skipping initialization');
      }
    }
  }
              ...
              ? profileController.userInfoModel != null
                  ? SafeArea(
                      ...
                      child: RefreshIndicator(
                        onRefresh: () async {
                          if (kDebugMode) {
                            debugPrint('🎁 [LoyaltyScreen] Pull to refresh triggered');
                          }
                          final loyaltyController = Get.find<LoyaltyController>();
                          final profileController = Get.find<ProfileController>();
