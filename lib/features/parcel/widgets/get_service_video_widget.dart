...
class _GetServiceVideoWidgetState extends State<GetServiceVideoWidget> {
  ...
  void initState() {
    ...
    final String url = widget.youtubeVideoUrl;
    if(url.isNotEmpty) {

      _controller = YoutubePlayerController(params: const YoutubePlayerParams(
        enableCaption: false, showVideoAnnotations: false,
      ));

      _controller.loadVideo(url);

      final String? convertedUrl = YoutubePlayerController.convertUrlToId(url);
