...
class SplashService implements SplashServiceInterface {
  ...
  Future<Response> getConfigData({required DataSourceEnum source}) async {
    final result = await splashRepositoryInterface.getConfigData(source: source);
  ...
  Future<LandingModel?> getLandingPageData({required DataSourceEnum source}) async {
    final result = await splashRepositoryInterface.getLandingPageData(source: source);
    return result is LandingModel? ? result : null;
  }

  @override
  Future<ModuleModel?> initSharedData() async {
    final result = await splashRepositoryInterface.initSharedData();
    return result;
  }
  ...
  Future<List<ModuleModel>?> getModules({Map<String, String>? headers, required DataSourceEnum source}) async {
    final result = await splashRepositoryInterface.getModules(headers: headers, source: source);
    return result is List<ModuleModel>? ? result : null;
  }
  ...
  Future<ResponseModel> subscribeEmail(String email) async {
    final result = await splashRepositoryInterface.subscribeEmail(email);
    return result is ResponseModel ? result : ResponseModel(false, 'Error subscribing email');
  }
