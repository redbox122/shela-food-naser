...
class FavouriteRepository implements FavouriteRepositoryInterface<ResponseModel> {
  ...
  Future<Response> getList({int? offset}) async {
    // Wishlist does not have a local cache; avoid 304/ETag to prevent empty UI
    return await apiClient.getData(AppConstants.wishListGetUri, useEtag: false);
  }

  @override
  Future<ResponseModel> add(dynamic a, {bool isStore = false, int? id}) async {
    ResponseModel responseModel;
    final Response response = await apiClient.postData('${AppConstants.addWishListUri}${isStore ? 'store_id=' : 'item_id='}$id', null, handleError: false);
  ...
  Future<ResponseModel> delete(int? id, {bool isStore = false}) async {
    ResponseModel responseModel;
    final Response response = await apiClient.deleteData('${AppConstants.removeWishListUri}${isStore ? 'store_id=' : 'item_id='}$id', handleError: false);
    if (response.statusCode == 200) {
      responseModel = ResponseModel(true, (response.body as Map<String, dynamic>)['message'] as String?);
    } else {
      responseModel = ResponseModel(false, response.statusText);
