...
class VerificationService implements VerificationServiceInterface {
  ...
  Future<ResponseModel> verifyPhone(String? phone, String? otp) async {
    final ResponseModel responseModel = await verificationRepoInterface.verifyPhone(phone, otp);
...
/*  @override
  ...
  Future<ResponseModel> verifyFirebaseOtp(
    ...
    if (isForgetPassPage) {
      responseModel =
          await verificationRepoInterface.verifyForgetPassFirebaseOtp(phoneNumber: phoneNumber, session: session, otp: otp);
    } else {
      responseModel = await verificationRepoInterface.verifyFirebaseOtp(
          phoneNumber: phoneNumber, session: session, otp: otp, loginType: loginType);
