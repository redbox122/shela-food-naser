...
class _NewUserSetupScreenState extends State<NewUserSetupScreen> {
  ...
  Future<void> _updatePersonalInfo(AuthController authController, String numberWithCountryCode) async {
    final String name = _nameController.text.trim();
    final BuildContext buildContext = context;
    final response = await authController.updatePersonalInfo(
      name: name.isNotEmpty ? name : widget.name,
      phone: (widget.phone != null && widget.phone!.isNotEmpty) ? widget.phone : numberWithCountryCode,
    ...
    if (response.isSuccess) {
      ...
      Get.find<LocationController>().navigateToLocationScreen(buildContext, 'sign-in', offNamed: true);
    } else {
