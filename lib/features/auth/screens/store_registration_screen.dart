...
import 'package:sixam_mart/features/auth/widgets/Map_searchInRiyadh_Zone.dart';
...
class _StoreRegistrationScreenState extends State<StoreRegistrationScreen>
  ...
  late bool _mapGestureActive = false;
  ...
  void initState() {
    ...
    for (final language in _languageList) {
      if (kDebugMode) {
        debugPrint(
            'StoreRegistration language loaded: key=${language.key}, value=${language.value}');
      }
    ...
    // Ensure zones and modules are loaded; inZone will be computed from real zone polygons
    Get.find<StoreRegistrationController>().getZoneList();

    // Initialize camera position only; do not force a zoneId that may not match
    Get.find<StoreRegistrationController>().setLocation(
        const LatLng(24.57934470964999, 46.5564288944006),
            ...
            Expanded(
              child: NotificationListener<ScrollNotification>(
                onNotification: (notification) {
                  // لو الإيماءات على الخريطة فعالة، نمنع التمرير للـ ScrollView
                  return _mapGestureActive;
                },
                                  ...
                                  child: Form(
                                    ...
                                    child: Column(
                                        ...
                                        children: [
                                          ...
                                          // SizedBox(
                                          //   height: 800,
                                          //   child: Center(child: MapSearchByNamePage()),
                                          // ),
                                          ...
                                          SizedBox(
                                            ...
                                            child: Listener(
                                              onPointerDown: (_) {
                                                setState(() {
                                                  _mapGestureActive =
                                                      true; // بدأ التفاعل مع الخريطة
                                                });
                                              },
                                              onPointerUp: (_) {
                                                setState(() {
                                                  _mapGestureActive =
                                                      false; // انتهى التفاعل مع الخريطة
                                                });
                                              },
                                              child: const RiyadhMapSearch(),
                                            ),
                                          ),
                ...
                CustomTextField(
                  ...
                  onChanged: (value) {
                    if (value != null && value.toString().isNotEmpty) {
                      ...
                      storeRegController.validPassCheck(value.toString());
                    } else {
                      if (storeRegController.showPassView) {
                        storeRegController.showHidePass();
                      }
      ...
      return Container(
        ...
        child: CustomButton(
          ...
          onPressed: () {
            ...
            debugPrint('  - inZone: ${storeRegController.inZone}');
            debugPrint(
                '  - restaurantLocation: ${storeRegController.restaurantLocation}');

            final bool isDisabled = (storeRegController.storeStatus == 0.1 &&
                    !ResponsiveHelper.isDesktop(context) &&
                    !storeRegController.inZone) ||
                (ResponsiveHelper.isDesktop(context) &&
                    !storeRegController.inZone);

            debugPrint('  - Button disabled: $isDisabled');
            ...
            final String minTime = storeRegController.storeMinTime;
            final String maxTime = storeRegController.storeMaxTime;
