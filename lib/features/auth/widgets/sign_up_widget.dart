...
class SignUpWidgetState extends State<SignUpWidget> {
            ...
            Padding(
              ...
              child: Column(mainAxisSize: MainAxisSize.min, mainAxisAlignment: MainAxisAlignment.center, children: [
                ...
                Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
                  ...
                  SizedBox(
                      width: Get.find<SplashController>().configModel!.refEarningStatus == 1 && isDesktop
                ...
                Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
                  ...
                  isDesktop
                      ? Expanded(
                          ...
                          inputAction: Get.find<SplashController>().configModel!.refEarningStatus == 1
                          ...
                          validator: (value) => ValidateCheck.validateConfirmPassword(value, passwordController.text),
                        ))
                      : const SizedBox()
                ]),
                SizedBox(height: isDesktop ? Dimensions.paddingSizeExtraLarge : Dimensions.paddingSizeLarge),
  ...
  void _register(AuthController authController, String countryCode) async {
    final SignUpBodyModel? signUpModel = await _prepareSignUpBody(authController, countryCode);

    if (signUpModel == null) {
      return;
    } else {
      authController.registration(signUpModel).then((status) async {
        _handleResponse(authController, status, countryCode);
      });
    }
  }

  void _handleResponse(AuthController authController, ResponseModel status, String countryCode) {
      ...
      if (status.authResponseModel != null && !status.authResponseModel!.isPhoneVerified!) {
        ...
        if (Get.find<SplashController>().configModel!.firebaseOtpVerification!) {
          Get.find<AuthController>()
              .firebaseVerifyPhoneNumber(numberWithCountryCode, status.message, CentralizeLoginType.manual.name);
        } else {
          if (ResponsiveHelper.isDesktop(context)) {
            Get.back();
        ...
        Get.find<LocationController>().navigateToLocationScreen(context, RouteHelper.signUp);
        if (ResponsiveHelper.isDesktop(context)) {
          Get.back();
        }
      }
  ...
  Future<SignUpBodyModel?> _prepareSignUpBody(AuthController authController, String countryCode) async {
    ...
    if (_formKeySignUp!.currentState!.validate()) {
      if (name.isEmpty) {
        ...
        final SignUpBodyModel signUpBody = SignUpBodyModel(
          name: name,
