...
class ExistingUserBottomSheet extends StatelessWidget {
    ...
    return Container(
      ...
      child: Column(mainAxisSize: MainAxisSize.min, children: [
        ...
        ResponsiveHelper.isDesktop(context) ? Align(
          ...
          child: IconButton(onPressed: ()=> Get.back<void>(), icon: const Icon(Icons.clear)),
        ) : Container(
          height: 5, width: 35,
          decoration: BoxDecoration(
                ...
                onPressed: () {
                  if(loginType == CentralizeLoginType.otp.name) {
                    authController.otpLogin(phone: number ?? email ?? '', loginType: loginType, otp: otp!, verified: 'no').then((response) {
                      ...
                      _responseHandle(context, response);
                    });
                  } else {
  ...
  void _responseHandle(BuildContext context, ResponseModel response) {
    if (!context.mounted) {
      return;
    }
    Get.back<void>();
    if(response.isSuccess && !response.authResponseModel!.isPersonalInfo!) {
      if(Get.context != null && ResponsiveHelper.isDesktop(Get.context!)) {
        Get.back<void>();
        Get.dialog<void>(NewUserSetupScreen(name: userModel.name??'', loginType: loginType, phone: number, email: email));
      } else {
        Get.toNamed<void>(RouteHelper.getNewUserSetupScreen(name: userModel.name??'', loginType: loginType, phone: number, email: email));
      }
    } else if(response.isSuccess && response.authResponseModel!.isPersonalInfo!) {
      // Get.offAllNamed(RouteHelper.getInitialRoute());
      Get.find<LocationController>().navigateToLocationScreen(context, 'sign-in', offNamed: true);
    } else {
