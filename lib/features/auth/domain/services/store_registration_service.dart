...
class StoreRegistrationService implements StoreRegistrationServiceInterface {
  ...
  Future<List<ZoneDataModel>?> getZoneList() async {
    final result = await deliverymanRegistrationRepositoryInterface.getList();
    return result is List<ZoneDataModel>? ? result : null;
  }
  ...
  Future<List<ModuleModel>?> getModules(int? zoneId) async {
    final result = await deliverymanRegistrationRepositoryInterface.getList(isZone: false, zoneId: zoneId);
