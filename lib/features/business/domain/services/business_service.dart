...
class BusinessService implements BusinessServiceInterface{
  ...
  Future<PackageModel?> getPackageList() async {
    final result = await businessRepoInterface.getList();
    return result is PackageModel? ? result : null;
  }
  ...
  Future<String> processesBusinessPlan(String businessPlanStatus, int paymentIndex, int storeId, String? digitalPaymentName, int? selectedPackageId) async {
    ...
    // }
    const String businessPlan = 'subscription';
    final int? packageId = selectedPackageId;
    final String? payment = paymentIndex == 0 ? 'free_trial' : digitalPaymentName;
    ...
    if(paymentIndex == 1 && digitalPaymentName == null) {
      ...
      businessPlanStatus = await setUpBusinessPlan(
        BusinessPlanBody(
          ...
          callBack: paymentIndex == 0 ? '' : (Get.context != null && ResponsiveHelper.isDesktop(Get.context!)) ? '$protocol//$hostname${RouteHelper.subscriptionSuccess}' : RouteHelper.subscriptionSuccess,
  ...
  Future<String> setUpBusinessPlan(BusinessPlanBody businessPlanBody, String? digitalPaymentName, String businessPlanStatus, int storeId) async {
    final Response response = await businessRepoInterface.setUpBusinessPlan(businessPlanBody);
    if (response.statusCode == 200) {
      if(response.body['redirect_link'] != null) {
        _subscriptionPayment(response.body['redirect_link']?.toString() ?? '', digitalPaymentName, storeId);
      } else {
