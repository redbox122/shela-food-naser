...
class ChatService implements ChatServiceInterface {
  ...
  Future<ConversationsModel?> getConversationList(int offset, String type) async {
    final result = await chatRepositoryInterface.getList(offset: offset, conversationList: true, type: type);
    return result is ConversationsModel? ? result : null;
  }

  @override
  Future<ConversationsModel?> searchConversationList(String name) async {
    final result = await chatRepositoryInterface.getList(searchConversationalList: true, name: name);
    return result is ConversationsModel? ? result : null;
  }

  @override
  Future<Response> getMessages(int offset, int? userID, String userType, int? conversationID) async {
    final result = await chatRepositoryInterface.getMessages(offset, userID, userType, conversationID);
  ...
  Future<Response> sendMessage(String message, String orderId, List<MultipartBody> images, int? userID, String userType, int? conversationID) async {
    final result = await chatRepositoryInterface.sendMessage(message, orderId, images, userID, userType, conversationID);
    return result is Response ? result : const Response();
  }
