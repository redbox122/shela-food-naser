...
class ChatController extends GetxController implements GetxService {
  ...
  Future<void> getConversationList(int offset, {String type = ''}) async {
    ...
    final ConversationsModel? conversationModel = await chatServiceInterface.getConversationList(offset, type);
    ...
    if(conversationModel != null) {
      ...
      final int index0 = chatServiceInterface.setIndex(_conversationModel!.conversations);
      final bool sender = chatServiceInterface.checkSender(_conversationModel!.conversations);
      _hasAdmin = false;
      if(index0 != -1 && (Get.context == null || !ResponsiveHelper.isDesktop(Get.context!))) {
        _hasAdmin = true;
        if(sender) {
  ...
  Future<void> searchConversation(String name) async {
    ...
    final ConversationsModel? searchConversationModel = await chatServiceInterface.searchConversationList(name);
    ...
    if (response != null && response.body['messages'] != {} && response.statusCode == 200) {
      if (offset == 1) {
        ...
        if(conversationID != null && _conversationModel != null && (Get.context == null || !ResponsiveHelper.isDesktop(Get.context!))) {
          final int index0 = chatServiceInterface.findOutConversationUnreadIndex(_conversationModel!.conversations, conversationID);
          if(index0 != -1) {
        ...
        final chatModel = ChatModel.fromJson(response.body as Map<String, dynamic>);
  ...
  void pickImage(bool isRemove) async {
    if(isRemove) {
      ...
      final List<XFile> imageFiles = await ImagePicker().pickMultiImage(imageQuality: 40);
