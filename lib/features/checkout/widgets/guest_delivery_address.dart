      ...
      child: Column(children: [
        Row(children: [
          ...
          takeAway
              ...
              : InkWell(
                  onTap: () async {
                    final String? previousGuestAddress = checkoutController.guestAddress?.address;
                    final address = await Get.toNamed(RouteHelper.getEditAddressRoute(checkoutController.guestAddress, fromGuest: true));

                    if (address != null && address is AddressModel) {
                      final addressModel = address;
                      checkoutController.setGuestAddress(addressModel);
                      if (previousGuestAddress != addressModel.deliveryAddress) {
                        checkoutController.getDistanceInKM(
                          LatLng(double.parse(addressModel.latitude ?? '0'), double.parse(addressModel.longitude ?? '0')),
                          LatLng(
                              double.parse(checkoutController.store!.latitude ?? '0'), double.parse(checkoutController.store!.longitude ?? '0')),
                        );
                      }
                    }
                  },
