  ...
  Widget build(BuildContext context) {
    return GetBuilder<ItemController>(builder: (itemController) {
      ...
      if(itemController.basicMedicineModel != null){
        ...
        for (final category in itemController.basicMedicineModel!.categories!) {
          categories.add(category);
        }
        for (final product in itemController.basicMedicineModel!.products!) {
          if(selectedCategory == 0) {
            products.add(product);
          }
          if (product.categoryIds != null && categories.isNotEmpty && selectedCategory < categories.length && product.categoryIds!.any((category) => category.id == categories[selectedCategory].id)) {
            products.add(product);
          }
        }
      }
