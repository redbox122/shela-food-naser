...
class Gallery3DState extends State<Gallery3D> with TickerProviderStateMixin, WidgetsBindingObserver, Gallery3DMixin {
  ...
  void animateTo(angle) {
    ...
    _scrollToAngle(angle as double);
  }

  @override
  void jumpTo(angle) {
    setState(() {
      _updateAllGalleryItemTransformByAngle(angle as double);
    });
  }
  ...
  void _scrollToAngle(double angle) {
    ...
    animation.addListener(() {
      setState(() {
        _updateAllGalleryItemTransformByAngle((animation.value - lastValue) as double);
        lastValue = animation.value as double;
      });
    });
  ...
  void _onFingerUp() {
    ...
    final double angle = controller.getTransformInfo(controller.currentIndex).angle;
    double targetAngle = 0;

    final offsetX = _lastUpdateLocation!.dx - _panDownLocation!.dx;
  ...
  void _updateAllGalleryItemTransform() {
    for (var i = 0; i < controller.getTransformInfoListSize(); i++) {
      final item = controller.getTransformInfo(i);
  ...
  void _updateWidgetIndexOnStack() {
    ...
    _tempList.clear();
    for (var i = 0; i < controller.getTransformInfoListSize(); i++) {
      final angle = controller.getTransformInfo(i).angle;
...
class Gallery3DController {
  ...
  void init(GalleryItemConfig itemConfig) {
    ...
    for (var i = 0; i < itemCount; i++) {
      final itemAngle = getItemAngle(i);
  ...
  double getItemAngle(int index) {
    final double angle = 360 - (index * unitAngle + 180) % 360;
    return angle;
  }
  ...
  void updateTransformByOffsetDx(double offsetDx) {
    final double offsetAngle = offsetDx / perimeter / 2 * 360;
  ...
  Offset calculateOffset(double angle) {
    final double width = widgetWidth * 0.7;
    final double radiusOuterX = width / 2;
    final double radiusOuterY = ellipseHeight;

    final double angleOuter = (2 * pi / 360) * angle;
    final double x = radiusOuterX * sin(angleOuter);
    final double y = radiusOuterY > 0 ? radiusOuterY * cos(angleOuter) : 0;
  ...
  double getOffsetAngleFormTargetIndex(int index) {
    final double targetItemAngle = getItemAngle(index) + baseAngleOffset;
