  ...
  Widget build(BuildContext context) {
    ...
    return GetBuilder<ItemController>(builder: (itemController) {
      ...
      if(widget.fromShop ? itemController.reviewedCategoriesList != null && itemController.reviewedItemList != null : itemController.basicMedicineModel != null){
        ...
        for (final category in widget.fromShop ? itemController.reviewedCategoriesList! : itemController.basicMedicineModel!.categories!) {
          categories.add(category);
        }
        for (final product in widget.fromShop ? itemController.reviewedItemList! : itemController.basicMedicineModel!.products!) {
          if(selectedCategory == 0) {
            products.add(product);
          }
          if(categories.isNotEmpty && selectedCategory < categories.length && product.categoryIds != null && product.categoryIds!.isNotEmpty && categories[selectedCategory].id == product.categoryIds![0].id){
            products.add(product);
          }
        }
      }
