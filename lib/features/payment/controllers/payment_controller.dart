...
class PaymentController extends GetxController implements GetxService {
  ...
  void changesMethod({bool canUpdate = true}) {
    final List<MethodInformations>? methodInformation = offlineMethodList?[selectedOfflineBankIndex].methodInformations;

    informationControllerList = [];
    informationFocusList = [];

    if (methodInformation != null && methodInformation.isNotEmpty) {
  ...
  Future<bool> saveOfflineInfo(String data) async {
    _isLoading = true;
    update();
    final bool success = await paymentServiceInterface.saveOfflineInfo(data);
    _isLoading = false;
    update();
    return success;
  }
  ...
  Future<bool> updateOfflineInfo(String data) async {
    _isLoading = true;
    update();
    final bool success = await paymentServiceInterface.updateOfflineInfo(data);
    _isLoading = false;
    update();
    return success;
  }
