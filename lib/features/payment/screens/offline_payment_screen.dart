...
class _OfflinePaymentScreenState extends State<OfflinePaymentScreen> {
  PageController pageController =
      PageController(viewportFraction: 0.85, initialPage: Get.find<PaymentController>().selectedOfflineBankIndex);
      ...
      for (int index = 0;
          index <
              Get.find<PaymentController>()
                  .offlineMethodList![Get.find<PaymentController>().selectedOfflineBankIndex]
                  .methodInformations!
                  .length;
          index++) {
    ...
    return Scaffold(
      ...
      body: SafeArea(
        child: GetBuilder<PaymentController>(builder: (paymentController) {
          final List<MethodInformations> methodInformation = paymentController.offlineMethodList != null
                      ...
                      child: SizedBox(
                        ...
                        child: Column(mainAxisSize: MainAxisSize.min, children: [
                          ...
                          SizedBox(
                            ...
                            child: PageView.builder(
                                ...
                                itemBuilder: (context, index) {
                                  final bool selected = paymentController.selectedOfflineBankIndex == index;
                                  return bankCard(context, paymentController.offlineMethodList, index, selected);
                                }),
                          ),
                          const SizedBox(height: Dimensions.paddingSizeLarge),
                          Text(
                            '${'amount'.tr} ${PriceConverter.convertPrice2(widget.total)}',
                          ...
                          ResponsiveHelper.isDesktop(context)
                              ? completeButton(KaidhaSubController, paymentController, methodInformation)
                              : const SizedBox(),
                        ]),
                      ),
                    ),
      ...
      child: CustomButton(
        ...
        onPressed: !allFieldsFilled
            ...
            : () async {
                ...
                if (complete) {
                  final String methodId = paymentController.offlineMethodList![paymentController.selectedOfflineBankIndex].id.toString();

                  String? orderId = '';
                  if (widget.forParcel) {
                    orderId = await Get.find<ParcelController>().placeOrder(widget.placeOrderBody, widget.zoneId, widget.total,
                        widget.maxCodOrderAmount, widget.fromCart, widget.isCashOnDeliveryActive,
                    ...
                    paymentController.saveOfflineInfo(jsonEncode(data)).then((success) {
                      if (success) {
                        Get.offAllNamed(RouteHelper.getOrderDetailsRoute(int.parse(orderId!),
                            fromOffline: true, contactNumber: widget.placeOrderBody.contactPersonNumber));
                      }
                    });
                  }
                } else {
              ...
              itemBuilder: (context, i) {
                return Padding(
                  ...
                  child: Row(children: [
                    ...
                    Flexible(
                        child: Text(offlineMethodList[index].methodFields![i].inputData!,
                            style: robotoMedium, overflow: TextOverflow.ellipsis)),
                  ]),
                );
              })
        ]),
