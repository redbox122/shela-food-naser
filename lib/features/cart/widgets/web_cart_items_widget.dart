...
class WebCardItemsWidget extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    // 🔥 PHASE 2.2: Web Cart Items with ID for partial rebuilds
                  ...
                  child: TextButton.icon(
                    onPressed: () {
                      ...
                      if (cartController.cartList.isEmpty) {
                        debugPrint('⚠️ WebCardItemsWidget: Cart is empty, cannot navigate to store');
                        return;
                      }
                      final firstItem = cartController.cartList[0].item;
                      if (firstItem?.moduleId == null || firstItem?.storeId == null) {
                        debugPrint('⚠️ WebCardItemsWidget: First item has no moduleId or storeId');
                        return;
                      }
                      
                      // 🔥 BUG FIX: After null check, we can safely use !
                      final moduleId = firstItem!.moduleId!;
                      final storeId = firstItem.storeId!;
                      
                      cartController.forcefullySetModule(context, moduleId);
                      Get.toNamed(
