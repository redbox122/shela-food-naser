...
class NotAvailableWidget extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    if (isStore && store != null) {
      final bool isOpen = Get.isRegistered<StoreController>()
    ...
    return Positioned(
      ...
      child: Container(
        ...
        child: Text(
          isStore
              ? store != null
                  ? (store!.storeOpeningTime == 'closed' || store!.storeOpeningTime == null)
                      ? 'closed_now'.tr
                      : '${'closed_now'.tr}${store!.active == true && store!.storeOpeningTime != null ? ' (${'open_at'.tr} ${DateConverter.convertRestaurantOpenTime(store!.storeOpeningTime!)})' : ''}'
                  : 'closed_now'.tr
